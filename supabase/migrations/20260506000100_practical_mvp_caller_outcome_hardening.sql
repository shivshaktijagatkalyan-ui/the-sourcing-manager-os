-- Harden caller outcome handling for Practical MVP operational UAT.
-- Keep outcomes safe, auditable, and aligned with sourcing-manager workflows.

ALTER TABLE public.leads_public
  DROP CONSTRAINT IF EXISTS leads_public_status_check;

ALTER TABLE public.leads_public
  ADD CONSTRAINT leads_public_status_check CHECK (
    lead_status IN (
      'new',
      'loan_active',
      'call_queued',
...
BEGIN
  -- 1. Verify caller has access (either assigned or has active data loan)
  IF NOT EXISTS (
    SELECT 1
  ...
  INSERT INTO public.audit_events (
    ...
    jsonb_build_object('outcome', p_outcome, 'is_terminal', v_is_terminal)
  );
