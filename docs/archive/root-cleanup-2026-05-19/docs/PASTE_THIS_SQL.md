# COPY THIS EXACT SQL TO SUPABASE DASHBOARD
...
3. Click: New Query
4. **COPY everything below** (from START to END)
...
1. You should see: "Query successful" message (green)
2. No red error messages
3. If successful, run: `node scripts/debug-authenticated-broker-rls.mjs`
...
**"Column already exists"** → Normal, just means it's updating. Continue.

**"Function already exists"** → Normal, CREATE OR REPLACE is updating it. Continue.

**"Policy already exists"** → Normal, we DROP before CREATE. Continue.

**Red errors at end** → Wait 30 seconds, run again. Database cache needs refresh.

---

Time: 2 minutes to paste and run.
