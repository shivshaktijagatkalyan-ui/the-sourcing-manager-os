...
class AddLeadFromBrokerScreen extends StatefulWidget {
  ...
  const AddLeadFromBrokerScreen({super.key, this.sourceBrokerId, this.sourceBrokerAlias});

  @override
  State<AddLeadFromBrokerScreen> createState() => _AddLeadFromBrokerScreenState();
}
...
class _AddLeadFromBrokerScreenState extends State<AddLeadFromBrokerScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_selectedBrokerId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a broker')));
      return;
    }
    ...
    try {
      if (AppConfig.isSupabaseConfigured) {
        ...
        final response = await client.functions.invoke('lead-from-broker', body: {
  ...
  void _showSuccess(String alias) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Lead "$alias" has been secured and connected to the broker.\n\nCustomer number encrypted rahega, screen par nahi dikhega.'),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Lead From Broker')),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              _textField(_aliasController, 'Lead Alias (e.g. L-542)', Icons.badge, validator: (v) => v!.isEmpty ? 'Alias required' : null),
              const SizedBox(height: 12),
              _textField(_phoneController, 'Customer Phone (Encrypted)', Icons.lock_outline, keyboardType: TextInputType.phone, helper: 'Saves directly to secure vault.'),
              const SizedBox(height: 12),
              _textField(_areaController, 'Lead Area', Icons.map_outlined),
              const SizedBox(height: 12),
              Row(
                children: [
                  Expanded(child: _textField(_budgetMinController, 'Budget Min', Icons.currency_rupee, keyboardType: TextInputType.number)),
                  const SizedBox(width: 12),
                  Expanded(child: _textField(_budgetMaxController, 'Budget Max', Icons.currency_rupee, keyboardType: TextInputType.number)),
                ],
              ),
              const SizedBox(height: 12),
              ...
              ElevatedButton(
                ...
                child: _isSubmitting ? const CircularProgressIndicator(color: Colors.white) : const Text('Secure Lead Intake', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
        ),
  ...
  Widget _brokerSelector() {
    if (_selectedBrokerId != null) {
      return Container(
        ...
        child: Row(
          children: [
            const Icon(Icons.person_pin, color: Color(0xFF6366F1)),
            const SizedBox(width: 12),
            Expanded(child: Text('Source: ${_selectedBrokerAlias ?? 'Selected Broker'}', style: const TextStyle(fontWeight: FontWeight.bold))),
            if (widget.sourceBrokerId == null)
              TextButton(onPressed: () => setState(() => _selectedBrokerId = null), child: const Text('Change')),
          ],
        ),
      );
    }
    ...
    return DropdownButtonFormField<String>(
      decoration: const InputDecoration(labelText: 'Source Broker', border: OutlineInputBorder(), prefixIcon: Icon(Icons.people_outline)),
      items: _brokers.map((b) => DropdownMenuItem(value: b['id'].toString(), child: Text(b['broker_alias']))).toList(),
      onChanged: (val) {
        setState(() {
          _selectedBrokerId = val;
          _selectedBrokerAlias = _brokers.firstWhere((b) => b['id'] == val)['broker_alias'];
        });
      },
    );
  }
